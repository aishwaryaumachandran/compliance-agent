...
flowchart TB
    subgraph User["👤 User"]
        UI["Simple Chat UI<br/>(React / Blazor)"]
    end
    ...
    subgraph AzureAI["Azure AI Services"]
        ...
        ContentUnderstanding["Azure AI<br/>Content Understanding<br/>(optional)"]
        AISearch["Azure AI Search<br/>(Compliance Knowledge Base)"]
    end

    subgraph Storage["Data & Storage"]
        Blob["Azure Blob Storage<br/>• Uploaded documents<br/>• KB source files"]
        CosmosDB["Azure Cosmos DB<br/>• Sessions<br/>• Case drafts<br/>• Audit log"]
    end
    ...
    ChatAgent -- "RAG query" --> AISearch
    AISearch -- "Semantic + keyword<br/>search" --> Embeddings
    AISearch -. "Index source" .-> Blob

    ExtractionAgent -- "Parse document" --> ContentUnderstanding
    ContentUnderstanding -. "Read file" .-> Blob
    UI -- "Upload" --> Blob

    ChatAgent -- "Read/write sessions" --> CosmosDB
    ExtractionAgent -- "Save drafts" --> CosmosDB
